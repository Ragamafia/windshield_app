...
#             )
#         else:
#             await message.answer(text="В базе нет автомобилей с не установленным уровнем сложности")
#
#
#     # async def _next_car(callback_query: CallbackQuery):
#     #     if car := await db.get_model_info():
#     #         level_none = next(group for group in car["groups"] if group.get("level") is None)
#     #         image_path = Path(cfg.path_to_images / car.get("brand") / car.get("model") / level_none.get("ids")[0] / "img.jpg")
...
#     #
#     #     else:
#     #         await callback_query.answer(text="В базе нет автомобилей с не установленным уровнем сложности")
#     #
#
...
#         #await _next_car(callback_query)
